...
import {Picker, AppRegistry, StyleSheet, TextInput, View, SafeAreaView, ScrollView, ImageBackground, keyboardAppearance, Image } from 'react-native'
  ...
  (user)=>{
    if(user){
      firebase.firestore().collection("users").doc(user.uid).set({
          ...
          subscribed:["marcopolo"],
          school: this.state.language,
...
render() {
    ...
    {this.props.subtitle && (
      ...
      <Card  borderRadius={15}>
        ...
        keyboardAppearance={'dark'}
        />
        <Text style={{paddingTop: 40, fontWeight: '400', fontSize: 20}}>Select your school</Text>
        <Picker
        pickerStyleType={{maxHeight: 20, }}
          selectedValue={this.state.language}
          onValueChange={(itemValue, itemIndex) => this.setState({language: itemValue})}>
          <Picker.Item label="UT Ausitn" value="UT Ausitn" />
          <Picker.Item label="UT Dallas" value="UT Dallas" />
          <Picker.Item label="Texas A&M" value="Texas A&M" />
        </Picker>
        <Button
